%% Gauss-Newton based SQP implementation using collocation with exact lifting
clc;
clear all;
close all;
...
%% MPCexport
acadoSet('problemname', ['GN_EN_' num2str(Nm)]);
...
if EXPORT
    mpc.exportCode( ['export_GN_EN_' num2str(Nm)] );
    copyfile('qpoases', ['export_GN_EN_' num2str(Nm) '/qpoases'], 'f')
end
if COMPILE
    copyfile('rien_solver_mex.c', ['export_GN_EN_' num2str(Nm) '/rien_solver_mex.c'], 'f')
    copyfile('make_rien_solver.m', ['export_GN_EN_' num2str(Nm) '/make_rien_solver.m'], 'f')
    cd(['export_GN_EN_' num2str(Nm)])
    make_rien_solver( ['../acado_GN_EN_' num2str(Nm) '_' num2str(d)] )
    cd ..
end
...
while KKT_val > 1e-12 && iter < 1000
    ...
    % Solve NMPC OCP
    evalc(['output = acado_GN_EN_' num2str(Nm) '_' num2str(d) '(input)']);
...
save(['../ACADO_GN_EN_' num2str(Nm) method '.mat'],'input','output','iter','simTime','qpTime','condenseTime','regularizeTime','cpuTime','outputs');
