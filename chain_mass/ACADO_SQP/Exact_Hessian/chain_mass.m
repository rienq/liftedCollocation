%% Exact Hessian based SQP implementation using collocation without lifting
clc;
clear all;
close all;
...
%% MPCexport
acadoSet('problemname', ['EH_' num2str(Nm)]);
...
if EXPORT
    mpc.exportCode( ['export_EH_' num2str(Nm)] );
    copyfile('qpoases', ['export_EH_' num2str(Nm) '/qpoases'], 'f')
end
if COMPILE
    copyfile('rien_solver_mex_INIS.c', ['export_EH_' num2str(Nm) '/rien_solver_mex.c'], 'f')
    copyfile('make_rien_solver.m', ['export_EH_' num2str(Nm) '/make_rien_solver.m'], 'f')
    cd(['export_EH_' num2str(Nm)])
    make_rien_solver( ['../acado_EH_' num2str(Nm) '_' num2str(d)] )
    cd ..
end
...
evalc(['output = acado_EH_' num2str(Nm) '_' num2str(d) '(input)']);
...
save(['../ACADO_EH_' num2str(Nm) method '.mat'],'input','output','iter','simTime','qpTime','condenseTime','regularizeTime','cpuTime','outputs');
